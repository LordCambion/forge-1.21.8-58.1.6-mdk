package net.lordcambion.mod3rnmod.event.render;
...
@Mod.EventBusSubscriber(modid = Mod3rnMod.MOD_ID, value = Dist.CLIENT, bus = Mod.EventBusSubscriber.Bus.MOD)
public class EnderHelmetShaderHandler {
    ...
    public static void registerShaders(RegisterEntitySpectatorShadersEvent event) {
        ...
        Mod3rnMod.LOGGER.info("Registered enderman spectator shader: invert");
    }
}

@Mod.EventBusSubscriber(modid = Mod3rnMod.MOD_ID, value = Dist.CLIENT)
class EnderHelmetTickHandler {
    ...
    static {
        try {
            ...
            Mod3rnMod.LOGGER.warn("Reflection setup failed, using fallback method");
        }
    }
        ...
        if (helmetActive) {
            ...
            if (reapplyCounter >= 20) { // Controlla ogni secondo (20 tick)
                if (!isShaderActive(mc)) {
                    Mod3rnMod.LOGGER.debug("Shader was disabled, re-applying");
                    activateEndermanShader(mc);
                }
                reapplyCounter = 0;
            }
    ...
    private static void activateEndermanShader(Minecraft mc) {
        try {
            ...
            if (postEffectIdField != null && effectActiveField != null) {
                ...
                Mod3rnMod.LOGGER.debug("Enderman shader activated directly: {}", shader);
                return;
            }
        } catch (Exception e) {
            Mod3rnMod.LOGGER.warn("Direct activation failed, using fallback");
        }

        // Fallback: usa il metodo vanilla
        try {
            mc.gameRenderer.checkEntityPostEffect(new EnderMan(EntityType.ENDERMAN, mc.level));
            Mod3rnMod.LOGGER.debug("Enderman shader activated via fallback");
        } catch (Exception e) {
            Mod3rnMod.LOGGER.error("Failed to activate enderman shader", e);
        }
    }
    ...
    private static void deactivateShader(Minecraft mc) {
        try {
            ...
            if (postEffectIdField != null && effectActiveField != null) {
                ...
                Mod3rnMod.LOGGER.debug("Shader deactivated directly");
                return;
            }
        } catch (Exception e) {
            Mod3rnMod.LOGGER.warn("Direct deactivation failed, using fallback");
        }

        // Fallback: usa il metodo vanilla
        try {
            mc.gameRenderer.clearPostEffect();
            Mod3rnMod.LOGGER.debug("Shader deactivated via fallback");
        } catch (Exception e) {
            Mod3rnMod.LOGGER.error("Failed to deactivate shader", e);
        }
    }
}